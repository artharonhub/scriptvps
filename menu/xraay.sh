...
END
  ...
  if [ "$(grep -w "vmess $user" "/root/.database" | wc -l)" != '0' ]; then
    sed -i "/vmess $user/d" "/root/.database"
  fi
...
function menu7() {
  ...
  #############
  get_AdminID=$(sed -n '2p' /root/ResBotAuth | cut -d' ' -f2)
  if [ "$(grep -w "vless $user" "/root/.database" | wc -l)" != '0' ]; then
    sed -i "/vless $user/d" "/root/.database"
  fi
...
function menu13() {
  ...
  #############
  get_AdminID=$(sed -n '2p' /root/ResBotAuth | cut -d' ' -f2)
  if [ "$(grep -w "xtls $user" "/root/.database" | wc -l)" != '0' ]; then
    sed -i "/xtls $user/d" "/root/.database"
  fi
