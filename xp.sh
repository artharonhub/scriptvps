...
for user in "${data[@]}"; do
    ...
    if [[ "$exp2" = "0" ]]; then
        ...
        harini=$(grep -E "^vmess " "/root/.database" | grep -w "$user $exp" | cut -d ' ' -f 4)
        uuid=$(grep -E "^vmess " "/root/.database" | grep -w "$user $exp" | cut -d ' ' -f 5)
        cust_id=$(grep -E "^vmess " "/root/.database" | grep -w "$user $exp" | cut -d ' ' -f 6)
        seller_id=$(grep -E "^vmess " "/root/.database" | grep -w "$user $exp" | cut -d ' ' -f 7)
        sed -i "/vmess $user $exp/d" "/root/.database"
        echo "vmess $user $exp $harini $uuid $cust_id $seller_id inactive" >>"/root/.database"
...
for user in "${data[@]}"; do
    ...
    if [[ "$exp2" = "0" ]]; then
        ...
        harini=$(grep -E "^vless " "/root/.database" | grep -w "$user $exp" | cut -d ' ' -f 4)
        uuid=$(grep -E "^vless " "/root/.database" | grep -w "$user $exp" | cut -d ' ' -f 5)
        cust_id=$(grep -E "^vless " "/root/.database" | grep -w "$user $exp" | cut -d ' ' -f 6)
        seller_id=$(grep -E "^vless " "/root/.database" | grep -w "$user $exp" | cut -d ' ' -f 7)
        sed -i "/vless $user $exp/d" "/root/.database"
        echo "vless $user $exp $harini $uuid $cust_id $seller_id inactive" >>"/root/.database"
    fi
done
...
for user in "${data[@]}"; do
    ...
    if [[ "$exp2" = "0" ]]; then
        ...
        harini=$(grep -E "^trojanws " "/root/.database" | grep -w "$user $exp" | cut -d ' ' -f 4)
        uuid=$(grep -E "^trojanws " "/root/.database" | grep -w "$user $exp" | cut -d ' ' -f 5)
        cust_id=$(grep -E "^trojanws " "/root/.database" | grep -w "$user $exp" | cut -d ' ' -f 6)
        seller_id=$(grep -E "^trojanws " "/root/.database" | grep -w "$user $exp" | cut -d ' ' -f 7)
        sed -i "/trojanws $user $exp/d" "/root/.database"
        echo "trojanws $user $exp $harini $uuid $cust_id $seller_id inactive" >>"/root/.database"
...
for user in "${data[@]}"; do
    ...
    if [[ "$exp2" = "0" ]]; then
        ...
        harini=$(grep -E "^xtls " "/root/.database" | grep -w "$user $exp" | cut -d ' ' -f 4)
        uuid=$(grep -E "^xtls " "/root/.database" | grep -w "$user $exp" | cut -d ' ' -f 5)
        cust_id=$(grep -E "^xtls " "/root/.database" | grep -w "$user $exp" | cut -d ' ' -f 6)
        seller_id=$(grep -E "^xtls " "/root/.database" | grep -w "$user $exp" | cut -d ' ' -f 7)
        sed -i "/xtls $user $exp/d" "/root/.database"
        echo "xtls $user $exp $harini $uuid $cust_id $seller_id inactive" >>"/root/.database"
